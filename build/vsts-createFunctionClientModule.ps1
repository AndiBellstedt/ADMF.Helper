...
$parentModule = 'ADMF.Helper'
if (-not $ModuleName) { $ModuleName = 'ADMF.Helper.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ADMF.Helper'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ADMF.Helper.psd1"
$prereqHash = @{
